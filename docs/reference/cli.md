...
The `lac` command starts actors, runs TypeScript clients, and issues local credentials. It is installed with the Node.js package. For a complete example, see the [chat tutorial](../../README.md#build-a-chat-room-in-your-terminal).
...
The development server looks for `src/actors.ts` in the current directory. The file must exist before startup and export named actor classes. See [actor definitions](api.md#actor) for class and export requirements.
...
Starts a server, loads the actor entrypoint, and registers it in the `local` namespace. Keep it running while using clients. Wait for the ready message before connecting:
...
- `--entrypoint <file>` — Actor file relative to the project. Defaults to `src/actors.ts`.
...
```sh
npx lac run --data-dir ./chat-state src/chat.ts Alice
npx lac run src/client.ts --room lobby --verbose
```
...
The child process receives `LAC_TOKEN`, `LAC_NAMESPACE_ID`, and `LAC_CONTROL_PLANE_URL`. Inherited `LAC_API_KEY` and `LAC_SOCKET_GATEWAY_URL` are removed from that process; other environment variables are inherited. Standard input, output, and error are inherited, so interactive clients work normally.
...
Configure the database, storage, authentication, and cloud provider using the [self-hosting guide](../guides/self-hosting.md). Every server setting and default is listed under [server configuration](../guides/self-hosting.md#server-configuration). The npm CLI defaults `LAC_PROCESS_ROLE` to `control_plane`; leave it unset for this use.
...
Root directory for downloaded runtimes. Ignored when `LAC_BINARY` is set.
...
```sh
npm install little-actors@latest
npx lac --help
```
...
The default cache path is `~/.cache/little-actors/<version>/<platform>-<arch>/`. Override its root with `LAC_CACHE_DIR`, or select an existing executable with `LAC_BINARY`.
