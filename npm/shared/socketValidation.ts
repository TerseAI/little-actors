...
import { jsonValueSchema, socketTagsSchema } from "./types.js"
...
function outgoingMessage(value: unknown, schemas: ActorSchemas = {}): JsonValue {
    ...
    if (isStateMessage(message)) throw new ActorProtocolError('socket message type "state" is reserved for the initial actor state')
    return message
}
...
function socketTags(tags: readonly string[], schemas: ActorSchemas = {}): string[] {
    ...
    for (const tag of result.data) {
        ...
        if (validated?.success === false) throw new ActorValidationError(`invalid socket tag: ${validated.error.message}`)
    }
    return result.data
}
