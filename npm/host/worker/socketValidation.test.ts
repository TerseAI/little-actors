...
import type { ActorMessageOf, ActorSocketOf } from "../../shared/actor.js"
import type { SocketConnection, WebSocketEventCommand } from "../../shared/types.js"
...
class ValidatedRoom extends Actor<z.infer<typeof metadata>, z.infer<typeof incoming>, z.infer<typeof outgoing>, z.infer<typeof tag>> {
...
test("incoming JSON is validated before onMessage and outgoing values are encoded automatically", async () => {
    calls.length = 0
    const runtime = new ActorRuntime(definition)
    const message = (value: unknown) => event({ type: "message", connection_id: connection.id, message: { type: "text", data: JSON.stringify(value) } })
    ...
    assert.deepEqual(reply, {
        ...
        state: {},
        effects: [{ type: "send", connection_id: "socket-1", message: { type: "text", data: '{"type":"posted","text":"hello","userId":"one"}' } }]
    })
...
test("invalid actor output, metadata, and tags cannot reach the gateway", async () => {
    ...
    for (const method of ["invalidOutput", "invalidMetadata", "invalidTags", "invalidFilter"]) {
        const reply = await runtime.handle({ type: "invoke", request_id: method, actor, method, args: [], state: null, connections: [connection] })
        assert.equal(reply.type, "failed")
    }
...
test("schema-validated tags persist and select broadcast recipients", async () => {
    const runtime = new ActorRuntime(definition)
    const reply = await runtime.handle({ type: "invoke", request_id: "join", actor, method: "joinMembers", args: [], state: null, connections: [connection] })
    ...
    assert.deepEqual(reply.effects, [
        ...
        { type: "broadcast", message: { type: "text", data: '{"type":"posted","text":"joined","userId":"one"}' }, except_connection_ids: [], tags: ["member"] }
    ])
})
...
test("restored connection tags are validated before a hibernated actor resumes", async () => {
    ...
    const runtime = new ActorRuntime(definition)
    const reply = await runtime.handle(event({ type: "message", connection_id: invalid.id, message: { type: "text", data: '{"type":"post","text":"hello"}' } }, [invalid]))
    assert.equal(reply.type, "failed")
    assert.deepEqual(calls, [])
})
