...
const executables = ["little-durable-objects", "little-durable-objects-modal-go"]
...
export class RuntimeInstaller {
    ...
    private async unpack(directory: string): Promise<void> {
        ...
        const url = `https://github.com/TerseAI/little-durable-objects/releases/download/v${version}/little-durable-objects-${platform}-${arch}.tar.gz`
        ...
        await x({ file, cwd: directory, strict: true, filter: (name, entry) => executables.includes(name) && "type" in entry && entry.type === "File" && entry.size <= maximumBytes })
        await rm(file)
        if (!(await completeBundle(directory))) throw new Error("Runtime archive is missing its runtime or bundled Modal provider.")
...
async function downloadRelease(url: string): Promise<Buffer> {
    ...
    if (!response.ok || !response.body) throw new Error(`Cannot download runtime (HTTP ${response.status}). Check your connection and that this package version has native release assets: ${url}`)
