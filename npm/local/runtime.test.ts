...
test("installs both executables from a verified release and reuses the cache offline", async t => {
    ...
    const options = { version: "1.2.3", platform: "darwin", arch: "arm64", cacheDirectory: path.join(directory, "cache") }
...
test("unsupported systems get an actionable error without downloading", async () => {
    const installer = new RuntimeInstaller({ version: "1.2.3", platform: "win32", arch: "x64", cacheDirectory: "unused" }, async () => {
